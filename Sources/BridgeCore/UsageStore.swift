...
public struct UsageTotals: Equatable {
    ...
    public var unknownCredits: Int64 { max(0, requests - creditReports) }
...
public final class UsageStore {
    ...
    public func record(_ event: UsageEvent) throws {
        ...
        let billing = event.nanoAiu.flatMap { value -> Double? in
    ...
    public func totals(today: Bool) throws -> UsageTotals {
        var s: OpaquePointer?
        let sql = "SELECT coalesce(sum(input),0),coalesce(sum(output),0),coalesce(sum(cached),0),coalesce(sum(requests),0),coalesce(sum(errors),0),coalesce(sum(unknown),0),coalesce(sum(nano_aiu),0),coalesce(sum(credit_reports),0) FROM totals"
    ...
    public func activity(ending: Date = Date(), weeks: Int = 26,
        ...
        var statement: OpaquePointer?
        let sql = """
        SELECT day,sum(input),sum(output),sum(cached),sum(requests),sum(errors),sum(unknown),sum(nano_aiu),sum(credit_reports)
